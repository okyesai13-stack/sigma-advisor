...
// Uses explicit version if available, otherwise falls back to a recent stable version
      ...
      if (parsedData) {
        ...
        if (parsedData.experience && Array.isArray(parsedData.experience) && parsedData.experience.length > 0) {
          const mappedExperiences = parsedData.experience.map((exp: {
            ...
            skills: Array.isArray(exp.skills) ? exp.skills.map(s => String(s)).join(', ') : '',
        ...
        if (parsedData.interests && Array.isArray(parsedData.interests) && parsedData.interests.length > 0) {
          setInterests(prev => {
            const newInterests = parsedData.interests
    ...
    try {
      ...
      const { error: profileError } = await supabase
        ...
        .upsert({
          id: user.id,
          user_type: userType,
          // Map user_type to valid goal_type to satisfy checking constraints ('learn', 'job', 'startup')
          // Since we removed explicit goal selection, we infer a default based on user persona
          goal_type: userType === 'student' ? 'learn' : 'job',
          ...
          <div className="space-y-8">
            ...
            <div className="text-center">
              ...
              <p className="text-muted-foreground">
                ...
                {(goalDescription || interests.length > 0) && (
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-center gap-3 max-w-lg mx-auto">
                ...
                <button
                  ...
                  className={`flex-1 p-3 rounded-lg border transition-all duration-200 text-left group relative flex items-center gap-3 ${userType === "student"
                    ? "border-primary bg-primary/5 shadow-sm"
                    : "border-border hover:border-primary/50 hover:bg-accent/50"
                    }`}
                >
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors shrink-0 ${userType === "student"
                  ...
                  <div>
                    <h4 className="font-medium text-foreground text-sm">Student</h4>
                ...
                <button
                  ...
                  className={`flex-1 p-3 rounded-lg border transition-all duration-200 text-left group relative flex items-center gap-3 ${userType === "working_professional"
                    ? "border-cyan-400 bg-cyan-50 shadow-sm"
                    : "border-border hover:border-cyan-400/50 hover:bg-cyan-50/50"
                    }`}
                >
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors shrink-0 ${userType === "working_professional"
                  ...
                  <div>
                    <h4 className="font-medium text-foreground text-sm">Professional</h4>
                    <p className="text-[11px] text-muted-foreground">
                      Employed / Seeking
                    </p>
                  </div>
